...
$query_rst = "SELECT * FROM users where Type='فني معمل'";
$rst = mysql_query($query_rst, $db_con) or die(mysql_error());
...
<div class="col-sm-9">
	<div class="panel panel-danger">
		<div class="panel-heading">
			<h3 class="panel-title"> المعامل</h3>
		</div>
		<div class="panel-body">
		...
		<div class="form-group">
			<label for="Employee">الموظف</label>
		      ...
		      <select name="Employee"> <?php do { ?><option><?php echo $row_rst['FName']; ?></option>
		      <?php } while ($row_rst = mysql_fetch_assoc($rst)); ?></select>
          </div>
		<div class="form-group">
			<label for="Building_No">رقم المبنى</label>
			<span id="sprytextfield4">
            <input type="text" class="form-control" id="Building_No" name="Building_No">
            <span class="textfieldRequiredMsg">مطلوب.</span><span class="textfieldInvalidFormatMsg">خطأ رفم.</span></span></div>
        <div class="form-group">
        ...
        <div class="form-group">
			...
			<span class="textfieldRequiredMsg">مطلوب</span></span></div>
         <div class="form-group">
			<label for="Status">الخدمة</label>
<select name="Status"><option value="1">في الخدمة</option><option value="0">خارج الخدمة</option></select>
        </div>
...
if(isset($_POST['Name'])and $_POST['Employee'] !="")
{
$fname=$_POST['Name'];
...
mysql_select_db($database_db_con, $db_con);
$sql=mysql_query("insert into labs values(null ,'$fname','$Employee','$Building_No','$Building_Name','$Hospital','$Status','$Notes')") or die(mysql_error());
$mmessage ="شكراً لك.";
