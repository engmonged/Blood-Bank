<?php require_once('Connections/db_con.php'); error_reporting(E_ERROR | E_PARSE);?>
<?php
...
$query_rst_Patient = "SELECT * FROM users where type='مريض'";
$rst_Patient = mysql_query($query_rst_Patient, $db_con) or die(mysql_error());
$row_rst_Patient = mysql_fetch_assoc($rst_Patient);
$totalRows_rst_Patient = mysql_num_rows($rst_Patient);
mysql_select_db($database_db_con, $db_con);
$query_rst_labo = "SELECT * FROM users where type='فني معمل'";
$rst_labo = mysql_query($query_rst_labo, $db_con) or die(mysql_error());
$row_rst_labo = mysql_fetch_assoc($rst_labo);
$totalRows_rst_labo = mysql_num_rows($rst_labo);
mysql_select_db($database_db_con, $db_con);
$query_rst_doc = "SELECT * FROM users where type='طبيب'";
$rst_doc = mysql_query($query_rst_doc, $db_con) or die(mysql_error());
$row_rst_doc = mysql_fetch_assoc($rst_doc);
$totalRows_rst_doc = mysql_num_rows($rst_doc);
mysql_select_db($database_db_con, $db_con);
$query_rst_dis = "SELECT * FROM diseases";
$rst_dis = mysql_query($query_rst_dis, $db_con) or die(mysql_error());
$row_rst_dis = mysql_fetch_assoc($rst_dis);
$totalRows_rst_dis = mysql_num_rows($rst_dis);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="panel panel-danger">
	<div class="panel-heading">
	<h3 class="panel-title"> نتائج الفحص الطبي</h3>
	</div>
<div class="panel-body">
	<form role="form"  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
		<div class="form-group">
		<label for="Name">اسم الفحص</label>
	...
	<label for="Patient">المريض</label>
	  ...
	  <select name="Patient">
	   <?php do { ?>
        <option value="<?php echo $row_rst_Patient['ID_NO']; $ppid=$row_rst_Patient['ID_NO'];?>"><?php echo $row_rst_Patient['FName']; ?></option>
          <?php } while ($row_rst_Patient = mysql_fetch_assoc($rst_Patient)); ?>
	    </select>
	</div>
	<div class="form-group">
<?php
if(isset($ppid)){
mysql_select_db($database_db_con, $db_con);
$query_rst_pationtsample = "SELECT * FROM blood_samples where PatientID='$ppid'";
$rst_pationtsample = mysql_query($query_rst_pationtsample, $db_con) or die(mysql_error());
$row_rst_pationtsample = mysql_fetch_assoc($rst_pationtsample);
$totalRows_rst_pationtsample = mysql_num_rows($rst_pationtsample);


}
?>
	<label for="sampleno">رقم العينة</label>
	
	  <select name="sampleno">
      <?php do { ?>
      <option><?php echo $row_rst_pationtsample['No']; ?></option>
	  <?php } while ($row_rst_pationtsample = mysql_fetch_assoc($rst_pationtsample)); ?>
      </select>
	</div>
	
	<div class="form-group">
	<label for="Official_Name">فني المعمل</label> <select name="Official_Name">
	<?php do { ?>
	 
	    <option><?php echo $row_rst_labo['FName']; ?></option>
	   
	  <?php } while ($row_rst_labo = mysql_fetch_assoc($rst_labo)); ?> </select>
	</div>
 ...
 <select name="Doctor_Name">
	<?php do { ?>
	    ...
	    <option><?php echo $row_rst_doc['FName']; ?></option>
	 
	  <?php } while ($row_rst_doc = mysql_fetch_assoc($rst_doc)); ?>
         </select>
         </div>
<div class="form-group">
	<label for="Diseases">المرض</label>
  <select name="Diseases">	<?php do { ?>
	    <option><?php echo $row_rst_dis['Name']; ?></option>
  	  <?php } while ($row_rst_dis = mysql_fetch_assoc($rst_dis)); ?> </select>
      </div>
	<div class="form-group">
	...
	<div class="form-group">
	<label for="Vacancies"> يستحق أجازة للأسباب</label>
	<textarea class="form-control" rows="5" id="Vacancies" name="Vacancies"></textarea>
	</div>
	<div class="form-group">
	...
	<button type="submit" class="btn btn-danger">أضف</button>
...
<?php
mysql_free_result($rst_Patient);

mysql_free_result($rst_pationtsample);

mysql_free_result($rst_dis);

mysql_free_result($rst_doc);

mysql_free_result($rst_labo);
?>
